...
Repacking the RAR files as ZIP drops the dependency altogether and makes every
comic behave the same way. Whatever tool you use, two things are worth getting
right: decide what to convert from the magic bytes rather than the extension, so
a `.cbr` that is already a ZIP is left alone; and reopen each new archive to
check it holds the same pages before replacing anything, keeping the originals
aside until you have looked at the result.

Store the pages rather than deflating them. Comic images are already compressed,
so deflating costs NAS cpu on every read and saves almost nothing.
